...
import 'package:ppdb_pangkalpinang/src/components/data/data_perzona_sd.dart';



class DaftarZonaSD extends StatefulWidget {
  const DaftarZonaSD({Key? key}) : super(key: key);

  @override
  State<DaftarZonaSD> createState() => _DaftarZonaSDState();
}

class _DaftarZonaSDState extends State<DaftarZonaSD> {
  @override
              ...
              sliver: SliverList(
                delegate: SliverChildBuilderDelegate(
                  (BuildContext context, int index) {
                    List<Widget> rows = [];
                    List<Widget> cells = [];
                    List listZonaSD = itemDataZonaSD[index].isiBody;
                    int isiBodyCount = itemDataZonaSD[index].isiBody.length;
                    for (int i = 0; i < isiBodyCount; i++) {
                        cells.add(
                          Text('- ${listZonaSD[i]}',style: TextStyle(color: ColorPallete.black80,fontSize: 12,fontWeight: FontWeight.w400),)
                        );
                    }
                  ...
                  childCount: itemDataZonaSD.length,
                ),
              ),
            ),
            // SliverPadding(
